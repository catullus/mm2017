...
reg2016$weekkey <- paste0(reg2016$Wteam,"_",reg2016$Week)
...
ranks2016 <- steph(select(reg2016, Week, Wteam, Lteam, Outcome), history=TRUE)

# first column is team number -- should convert this from wide to long
# paste 
dim(ranks2016$history)

row.names(ranks2016$history)

library(tidyr)
# matrix comes in wide format
wide_ranks <- as.data.frame(ranks2016$history)
wide_ranks$player <- rownames(wide_ranks)

# convert to long format
long_ranks <- gather(wide_ranks, value = player)
names(long_ranks) <- c("player", "type", "value")
long_ranks$week <- stringr::str_sub(long_ranks$type,1,1)

# only grab ratings
weekly_ranks <- filter(long_ranks, str_detect(type, "Rating"))
weekly_ranks$key <- paste0(weekly_ranks$player,"_",weekly_ranks$week)

merge(reg2016, weekly_ranks, "weekkey", "key")
